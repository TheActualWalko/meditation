...
$(function(){
  ...
  function init(){
    $("[data-vg-sound-player]").each(function(){
      ...
      var startFrom = $(this).attr('data-vg-start-from');
      if (startFrom) {
        function seekCallback() {
          console.log('seeking');
          player.seek(startFrom);
          player.audioElement.removeEventListener('canplaythrough', seekCallback);
        };
        player.audioElement.addEventListener('canplaythrough', seekCallback);
      }
    });
  }
